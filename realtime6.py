...
async def receive_audio(websocket, output_stream):
    """
    サーバからのテキスト/音声応答を受信する処理。
    - 「回答が二重になる」を防ぐため、partial_transcriptを保持し差分だけ出力。
    - エラーでも強制終了しない。
    """
    loop = asyncio.get_event_loop()
    partial_transcript = ""       # 出力済みテキストのバッファ
    response_in_progress = False  # 応答中かどうか
    ...
    while True:
        ...
        # --- ツール呼び出しイベント ---
        if (
            response_data.get("type") == "response.function_call_arguments.done"
        ):
...
async def send_audio(websocket, stream, CHUNK):
    """
    マイクから常時音声を取得。
    サイレンスを検出したら「ユーザの発話が終わった」とみなし、
    commit & response.create を送る。
    
    - ユーザが話さない限りは commit を送らないのでサーバから回答は返らない。
    - ハウリングが起きやすい場合はヘッドフォンを使用、あるいは閾値を上げる/物理的にマイクとスピーカーを離す。
    """
    timing_info["audio_capture_start"] = time.time()

    # ハウリング・ループを防ぐため閾値を高めに設定
    SILENCE_THRESHOLD = 1200   # これ未満なら「ほぼ無音」
    MIN_SILENCE_CHUNKS = 20    # 無音が何フレーム続いたら発話終了とみなすか
    ...
    while True:
        ...
        # 音量チェック
        volume = audioop.rms(audio_data, 2)  # 16bitデータをrms
        is_silent = (volume < SILENCE_THRESHOLD)

        # 送る前に「ユーザが実際に喋っているかどうか」判定
        if in_session:
            # すでに発話中
            if is_silent:
                silence_count += 1
